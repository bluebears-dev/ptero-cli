...
pub trait StegoMethod<Method> {
    fn hide(&self, data: &mut dyn Iterator<Item = Bit>, method: &mut Method) -> Result<String, Box<dyn Error>>;
    fn reveal(&self, method: Method) -> Result<Vec<Bit>, Box<dyn Error>>;
}